...
export default function PatientTicketStatus() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minHeight: '100vh', padding: '40px 24px', backgroundColor: '#FAFAFA' }}>
      <div style={{ width: '100%', maxWidth: '800px', marginBottom: '24px' }}>
        <Link to="/patient" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', color: 'var(--saas-accent)', textDecoration: 'none', fontWeight: 600, marginBottom: '16px' }}>
          <ArrowLeft size={16} /> Back to Dashboard
        </Link>
        <h1 style={{ fontSize: '2rem', fontWeight: 800, color: 'var(--saas-fg)', margin: 0 }}>Screening Status</h1>
      </div>
      ...
      <div style={{ width: '100%', maxWidth: '800px', backgroundColor: '#fff', borderRadius: '16px', border: '1px solid var(--saas-border)', padding: '32px', marginBottom: '24px', boxShadow: '0 4px 12px rgba(0,0,0,0.03)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
          <div>
            <h2 style={{ fontSize: '1.25rem', fontWeight: 700, margin: '0 0 8px 0' }}>Your Case</h2>
            <div style={{ color: 'var(--saas-fg-muted)', fontSize: '0.9rem' }}>Assigned Doctor: Dr. {ticket.doctor_name}</div>
          </div>
          <div style={{ padding: '8px 16px', borderRadius: '24px', backgroundColor: isAccepted ? 'rgba(34,197,94,0.1)' : 'rgba(245,158,11,0.1)', color: isAccepted ? '#22c55e' : '#d97706', display: 'flex', alignItems: 'center', gap: '6px', fontWeight: 600 }}>
            ...
            {isAccepted ? "Doctor Accepted" : "Under Review"}
          </div>
        </div>
        ...
        <div style={{ borderTop: '1px solid var(--saas-border)', paddingTop: '24px', marginBottom: '24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginTop: '24px' }}>
            <div style={{ padding: '16px', backgroundColor: '#f9fafb', borderRadius: '8px', border: '1px solid var(--saas-border)' }}>
              <div style={{ fontSize: '0.8rem', color: 'var(--saas-fg-muted)', marginBottom: '4px' }}>Scan Quality</div>
              <div style={{ fontSize: '1rem', fontWeight: 600, color: ticket.quality_label === 'REJECT' ? '#ef4444' : ticket.quality_label === 'BORDERLINE' ? '#d97706' : '#22c55e' }}>
                {ticket.quality_label || "GOOD"} 
              </div>
              {ticket.enhancement_used && (
                <div style={{ fontSize: '0.8rem', color: 'var(--saas-accent)', marginTop: '4px' }}>
            ...
            <div style={{ padding: '16px', backgroundColor: '#f9fafb', borderRadius: '8px', border: '1px solid var(--saas-border)' }}>
              <div style={{ fontSize: '0.8rem', color: 'var(--saas-fg-muted)', marginBottom: '4px' }}>Action Required</div>
              <div style={{ fontSize: '1rem', fontWeight: 600, color: ticket.refer ? '#ef4444' : '#22c55e' }}>
                {ticket.refer ? 'Consultation Needed' : 'No Action Required'}
              </div>
            </div>
          </div>
        </div>
      ...
      {isAccepted ? (
        ...
        <div style={{ width: '100%', maxWidth: '800px', textAlign: 'center', padding: '40px', color: 'var(--saas-fg-muted)', backgroundColor: '#fff', border: '1px dashed var(--saas-border)', borderRadius: '16px' }}>
          Chat will be available once the doctor accepts and schedules a consultation.
        </div>
      )}
    </div>
  );
